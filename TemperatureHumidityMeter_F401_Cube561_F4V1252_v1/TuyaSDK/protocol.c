...
void uart_transmit_output(unsigned char value)
{
	...
	uart_value = value;
	HAL_UART_Transmit(&huart1, &uart_value, 1, 1000 );
...
unsigned long TemperatureCycleReport = 1;
unsigned long HumidityPeriodicReport = 1;
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE, BatteryLevel); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_CHARGE_STATE, ChargeState); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,TemperatureCycleReport); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,HumidityPeriodicReport); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH,SwitchState); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_LED,LedState); //BOOL型数据上报;
...
{
	AirConditionerTypeDef A_Test;
	unsigned char error = 0;
	
	A_Test.A_Mode = HEAT;
	A_Test.A_WindSpeed = WAUTO;
	A_Test.A_Temperature = 20;
    ...
    if(switch_1 == 0) {
        ...
        //开关开
			SwitchState = 1;
			printf("switch = %d\r\n", switch_1);
			error = AirConditioner_Set(A_Test);
			printf("\r\nIR TX data error = %d\r\n", error);
    }
...
/*****************************************************************************
函数名称 : dp_download_led_handle
功能描述 : 针对DPID_LED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    led = mcu_get_dp_download_bool(value,length);
    if(led == 0) {
        //开关关
			LedState = 0;
			printf("led = %d\r\n", led);
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LED,led);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_LED:
            //LED测试处理函数
            ret = dp_download_led_handle(value,length);
        break;
